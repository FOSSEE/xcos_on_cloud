function CSCOPXY3D() {
    ...
    CSCOPXY3D.prototype.set = function CSCOPXY3D() {
        ...
        if(size(wpos_1,"*") != 0 && size(this.wpos,"*") != 2){
        ...
        if(this.N < 2){
            for(var i = 0; i < size(clrs_1,1); i++){
                for(var j = 0;j < size(clrs_1,2); j++){
                    if(clrs_1[i][j] <= 0){
        ...
        this.x.model.rpar = new ScilabDouble(...vec_x_1, ...vec_y_1, ...vec_z_1, ...param3ds_1);
